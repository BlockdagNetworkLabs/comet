...
/**
 * @dev Custom Timelock for more realistic deployments and scenarios.
...
contract CustomTimelock {
    ...
    function executeTransaction(address target, uint value, string memory signature, bytes memory data, uint eta) external payable returns (bytes memory) {
        ...
        //ATTENTION: This was modified. The signature is already encoded in the data
    ...
    // function executeNoQueuedTransaction(address target, uint value, bytes calldata data) external payable returns (bool success, bytes memory returnData) {
    //     if (msg.sender != admin) revert Unauthorized();
